...
export function KakaoCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (hasProcessed.current) {
      console.log('[KakaoCallback] 이미 처리됨 - 스킵');
      return;
    }
    ...
    const handleKakaoCallback = async () => {
      ...
      // 에러가 있는 경우
      if (errorParam) {
        const errorMessage = '카카오 로그인이 취소되었습니다.';
      ...
      // 인증 코드가 없는 경우
      if (!code) {
        const errorMessage = '카카오 인증 코드가 없습니다.';
        setError(errorMessage);
        toast.error(errorMessage);
        setTimeout(() => navigate('/login'), 2000);
        return;
      ...
      try {
        // 백엔드로 인증 코드 전송
        console.log('[KakaoCallback] 인증 코드 전송:', code);
        const response = await authApi.handleKakaoCallback(code);
        console.log('[KakaoCallback] 백엔드 응답:', response);

        if (response.success) {
          console.log('[KakaoCallback] 로그인 성공, 토큰:', response.token);
          console.log('[KakaoCallback] 사용자 정보:', response.user);

          // 인증 상태 확인
          const isAuth = authApi.isAuthenticated();
          console.log('[KakaoCallback] 인증 상태:', isAuth);

          toast.success('카카오 로그인에 성공했습니다!');

          // 약간의 지연 후 리다이렉트 (auth-change 이벤트 처리 대기)
          setTimeout(() => {
            console.log('[KakaoCallback] 홈으로 이동');
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
        {error ? (
          ...
          <>
            ...
            <h2 className="text-xl font-semibold mb-2">카카오 로그인 중...</h2>
